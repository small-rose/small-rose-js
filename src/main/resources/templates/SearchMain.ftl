        ...
        <div class="ui  small  message" style="margin-top: 50px;opacity: 0.7">
            <div class="content">
                ...
                <ul class="list small">
                    <li>每日自动抓取前一日排名。</li>
                    <li>支持拼音搜索。</li>
                    <li>简书昵称以最新昵称为准,昵称改动会有数据延迟。</li>
                    <li>搜索结果显示最近30次的上榜记录。</li>
...
<script>
    ...
    function initSearchInput() {
        $('#jarSearch.search.dropdown').dropdown({
            ...
            action: function (text, value, element) {
                ...
                openRankList(value.split(":")[0], value.split(":")[1]);
                $("#searchResult").show();
            },
    ...
    function openRankList(nickName, nickNamePy) {
        var checkbox ;
        nickName = encodeURIComponent(nickName);
        nickNamePy = encodeURIComponent(nickNamePy);
        // 清空原内容
        $("#searchResult").empty();
        $("#searchResult").addClass("loading");
        $("#searchResult")
            .load("/rankList?nickName=" + nickName + "&nickNamePy=" + nickNamePy,
                null,
